...
import { createLabel, listLabelsByBoard } from "@/lib/store";
...
export async function GET(_req: Request, { params }: Ctx) {
  try {
    ...
    const etiquetas = await listLabelsByBoard(boardId);
...
export async function POST(req: Request, { params }: Ctx) {
  try {
    ...
    const etiqueta = await createLabel(
      boardId,
      body.nome,
      body.cor ?? CORES_ETIQUETA[0]
    );
